      ...
      <div style={{ maxWidth: maxW, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ padding: "24px 28px" }}>
            ...
            {!data.description && !data.contactEmail && !data.contactPhone && !data.socialTwitter && !data.socialInstagram && !data.socialFacebook && !data.socialLinkedin && (
              <div style={{ background: subtleBg, border: `1px dashed ${border}`, borderRadius: radius.md, padding: "20px 16px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: 11, color: textFaint, margin: 0, lineHeight: 1.5 }}>
                  Fill in description, contact info, and social links in <strong>Step 4: Settings</strong> — they'll appear here.
                </p>
              </div>
            )}

            {/* Gallery images */}
            {(data as any).galleryImages && (data as any).galleryImages.length > 0 && (
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: (data as any).galleryImages.length === 1 ? "1fr" : "1fr 1fr", gap: 6 }}>
                  {(data as any).galleryImages.slice(0, 4).map((img: string, i: number) => (
  ...
  return (
    <div style={{ background: pageBg, width: "100%", minHeight: "100%", fontFamily: font, padding: "12px 10px" }}>
      <div style={{ background: cardBg, borderRadius: radius.lg, overflow: "hidden", border: `1px solid ${border}` }}>
        ...
        {/* Product info section - shown above payment on mobile */}
        {(data.description || data.contactEmail || data.contactPhone || (data as any).galleryImages?.length > 0) && (
          <div style={{ padding: "14px 16px", borderBottom: `1px solid ${border}` }}>
            {data.description && (
              <div style={{ marginBottom: 14 }}>
                <p style={{ fontSize: 10, fontWeight: 700, color: textFaint, textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 6px" }}>About</p>
                <p style={{ fontSize: 12, color: text, lineHeight: 1.6, margin: 0, whiteSpace: "pre-wrap" }}>{data.description}</p>
              </div>
            )}
            {(data as any).galleryImages?.length > 0 && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4 }}>
                {(data as any).galleryImages.slice(0, 2).map((img: string, i: number) => (
                  <div key={i} style={{ aspectRatio: "4/3", borderRadius: radius.sm, background: `url(${img}) center/cover`, border: `1px solid ${border}` }} />
                ))}
              </div>
            )}
        ...
        {(data.contactEmail || data.contactPhone) && (
          <div style={{ borderTop: `1px solid ${border}`, padding: "12px 16px", background: subtleBg }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: textFaint, textTransform: "uppercase", margin: "0 0 6px" }}>Contact</p>
            {data.contactEmail && <ContactLine icon="✉" label={data.contactEmail} color={text} brand={data.brandColor} small />}
            {data.contactPhone && <ContactLine icon="☎" label={data.contactPhone} color={text} brand={data.brandColor} small />}
          </div>
        )}
