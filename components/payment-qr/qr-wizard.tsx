...
export function QrWizard({
  initialData = DEFAULT_QR, initialStep = 0, editing = false, onBack, onSaveDraft, onPublish, onSyncState,
}: {
  initialData?: QrData; initialStep?: number; editing?: boolean;
  onBack: () => void; onSaveDraft?: (data: QrData, step: number) => void; onPublish?: (data: QrData) => void;
  ...
  const advance = () => {
    if (currentStep === totalSteps - 1) {
      ...
      setErrors([]); onPublish?.(data); setPhase("success");
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        <div style={{ width: 480, display: "flex", flexDirection: "column", borderRight: `1px solid ${C.border}`, background: C.white, flexShrink: 0 }}>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, padding: "14px 24px", display: "flex", alignItems: "center", gap: 10, background: C.white, flexShrink: 0 }}>
            ...
            {onSaveDraft && (
              <button onClick={() => canSaveDraft && onSaveDraft(data, currentStep)} disabled={!canSaveDraft} title={canSaveDraft ? undefined : "Name the QR first to save a draft"}
